...
# Name: Bump_Serial
# Type: method
# Description: Increments serial number for a zone
# Syntax: $zone = $obj->Bump_Serial([zone => $zone], [host => $host], [address => $addr]);
# End-Doc
sub Bump_Serial {
    my $self = shift;
    my %opts = @_;
    ...
    foreach my $bumpzone (@zones) {
        my $qry = "update dns_soa set tstamp=now(),serial=serial+1 where zone=?";
        my $cid = $dbcache->open($qry);
        $db->SQL_ExecQuery( $cid, $zone ) || $db->SQL_Error( $qry . " ($zone)" );
    }
}
